...
road_images=np.array(road_images)
...
def build_custom_unet(input_shape):
    ...
    """ Encoder """
    s1 = encoder_block(inputs, 32)         ## (512 x 512)
    s2 = encoder_block(s1, 64)            ## (256 x 256)
    s3 = encoder_block(s2, 64)           ## (128 x 128)
    s4 = encoder_block(s3, 128)             ## (64 x 64)

    """ Bridge """
    b1 = conv_block(s4,256)         ## (32 x 32)
    b1 = MaxPooling2D(2)(b1)
    b1 = conv_block(b1,256)
    """ Decoder """
    ...
    d6 = conv_block(d5, 32)
    """ Output """
    outputs = Conv2D(3, 1, activation="sigmoid")(d6)
...
unet_model=build_custom_unet(img_shape)
earlystopper = EarlyStopping(patience=100, verbose=1)
checkpointer = ModelCheckpoint('model_unet_checkpoint.h5', verbose=1, save_best_only=True)

results = unet_model.fit(road_images, mask_images ,validation_split=0.2, batch_size=1, epochs=1,
...
plt.show()
plt.imshow(mask_images[img_id])
plt.show()
new_img = new_img.reshape(1,128,512,3)
